       ...
       if(class(con_openmrs)[1]=="MySQLConnection"){
        if(dbIsValid(con_openmrs)){
          ...
          if(nrow(location)==1){
            ...
            # Get Patient data
            sql_query_patients <- createSqlQueryPatient(param.location = location_id,param.end.date = curr_date)
            patients <- getOpenmrsData(con.openmrs = con_openmrs,query = sql_query_patients)
            ...
            saveProcessLog(mpi.con = con_mpi,process.date = curr_datetime,process.type = 'get location info',affected.rows = 0,
