...
# clear log file
log_file <- 'logs/log_file_patient_atribute.txt'
      ...
      if(class(con_openmrs)[1]=="MySQLConnection"){
        if(dbIsValid(con_openmrs)){
          ...
          if(nrow(df_openmrs_patient_pat_attribute) >0 ) {
            ...
            saveProcessLog(mpi.con = con_mpi,process.date = curr_datetime,process.type = 'Fetch patient attribute info',affected.rows = 0,
            ...
            writeLog(file = log_file,msg =paste0("-- Fetch patient attribute info for DB: ", db_name, " took ", elapsed_time))
            writeLog(file = log_file,msg =paste0("-------------------------------------------------------------------------------------------"))
            print(paste0("-------------------------------------------------------------------------------------------"))
            print(paste0("-- Fetch patient attribute info for DB: ",db_name, " took ", elapsed_time))
            print(paste0("-------------------------------------------------------------------------------------------"))
